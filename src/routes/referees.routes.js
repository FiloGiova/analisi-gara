...
refereesRouter.get('/seasons', (req, res) => {
  requireRefereeInspection(req);
  res.json({ seasons: listSeasons({ competitions: scopedCompetitions(req) }) });
});

refereesRouter.get('/ranking', (req, res) => {
  requireRefereeInspection(req);
  res.json({
    ranking: getRefereeRanking({
...
refereesRouter.get('/:id/progress', asyncHandler(async (req, res) => {
  ...
  if (req.user?.role === 'referee') {
    ...
    getReferee(id, { competitions: scopedCompetitions(req) });
  }
  res.json({ progress: getRefereeProgress(id, { season: String(req.query.season || '').trim() }) });
}));

refereesRouter.get('/:id', asyncHandler(async (req, res) => {
  requireRefereeInspection(req);
  res.json({
    referee: getReferee(Number(req.params.id), {
      season: String(req.query.season || '').trim(),
      competitions: scopedCompetitions(req)
    })
  });
...
refereesRouter.post('/', requireAdmin, asyncHandler(async (req, res) => {
  const referee = createReferee(req.body);
  res.status(201).json({ referee });
}));

refereesRouter.put('/:id', requireAdmin, asyncHandler(async (req, res) => {
  const referee = updateReferee(Number(req.params.id), req.body);
  res.json({ referee });
}));

refereesRouter.get('/:id/rosters', asyncHandler(async (req, res) => {
  requireRefereeInspection(req);
  getReferee(Number(req.params.id), { competitions: scopedCompetitions(req) });
  res.json({ rosters: listRosters(Number(req.params.id)) });
}));

refereesRouter.post('/:id/rosters', requireAdmin, asyncHandler(async (req, res) => {
  const rosters = addRoster(Number(req.params.id), {
...
refereesRouter.delete('/:id/rosters/:rosterId', requireAdmin, asyncHandler(async (req, res) => {
  removeRoster(Number(req.params.id), Number(req.params.rosterId));
  res.json({ ok: true });
}));
